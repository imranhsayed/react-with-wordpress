...
### 3. [build-app-for-heroku](https://github.com/imranhsayed/react-with-wordpress/tree/build-app-for-heroku)

Contains features of [jwt-verify-with-node](https://github.com/imranhsayed/react-with-wordpress/tree/jwt-verify-with-node) and its 
an app ready to deploy to heroku

## Commands

- `start` Runs node server for development ( in watch mode ).
The `server.js` sends all front end route request to index.html
and then all front end route requests is handled by reach router
